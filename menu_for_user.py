...
def list_functions_user_choose(user_choose: int, user_directory: str,
    ...
    if user_choose < 1 or user_choose > 7:
    ...
    # if user_Choose is 7, the function observe the number of different suffix of secured file!!!
    # by dictionary
    elif user_choose == 7:

        file_name = input("Enter file name that you want marks as unsecured:")
        dict_secured_file = get_dict_of_secured_files(get_dict_files_details(user_directory),
                                                      dict_unsecured_files_from_main)


        restore_secured_file(dict_secured_file, dict_unsecured_files_from_main, file_name)

        print(dict_unscred_file)
